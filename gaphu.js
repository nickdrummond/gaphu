/****************************************************************
 *
 * gaphu.js javascript library
 * http://www.nickdrummond.co.uk/gaphu.js/
 *
 * requires: jquery-1.4.3.js (http://jquery.com/)
 * requires: gaphu-x.x.x.css
 *
 * version:  v0.0.9 beta
...
function ExpressionEditor(editorId, userOptions){
    ...
    function requestAutocomplete(expression) {
        makeRequest(REQ_AUTOCOMPLETE, expression, that.options.autocomplete, handleAutocomplete, that.autocompleteCache);
    }
    ...
    function requestParse(expression) {
        ...
        makeRequest(REQ_PARSE, expression, that.options.parser, handleParse, that.parseCache);
    }
    ...
    function makeRequest(name, expression, method, callback, responseCache){
        ...
        if (response){
            ...
            $.ajax({
                ...
                dataFilter: function(data, type){     // show the raw response text
                    showResponse(data);
                    return data;
                },
                success: function(data, textStatus, request){
                    ...
                    var response = getResponse(this, data);
    ...
    function getSpanPos(span){
        ...
        var pos = {};
        if (span.length == 1){
            // cannot simply use offset for span in case the editor is inside a positioned container (as all absolute pos are relative to offsetParent)
            // TODO: need to add offsets for border + margin - padding included in .position() result
            var spanPos = span.position(); // relative to inside top left of editor border (editor is positioned)
            pos.x = editorPosition.left + spanPos.left;
            pos.y = editorPosition.top + spanPos.top + span.height();
        }
        else{
    ...
    function handleAjaxError(request, textStatus, errorThrown){
        ...
        debug(that.editorId + " (" + request.statusText + "/" + textStatus + "):\n" + errorThrown);
    }
...
function getResponse(request, data){
    if (data.expr){ // then this is a json object
        return handleJSON(request, data);
    }
    else{
        if (typeof data == 'string'){  // if this is plain text XML
            data = parseXML(data);     // if so, convert it into a dom
        }

        return handleXML(request, data);
    }
}

function handleXML(request, data) {

    if (request == REQ_PARSE){
        var root = $("error", data);
    ...
    else if (request == REQ_AUTOCOMPLETE){
        root = $("results", data);
        if (root.length > 0){
    ...
    throw new Error("Unknown XML response: " + data.child);
}

function handleJSON(request, data) {
    if (request == REQ_PARSE){
        if (data.token){ // then this is an error
            return new ParseError(data.expr, data.message, data.pos, data.token);
        }
        else {
            return new ParseSuccess(data.expr, data.message);
        }
    }
    else if (request == REQ_AUTOCOMPLETE){
        var result = new AutocompleteResult(data.expr, data.pos, data.token);
        result.expectedByType = data.expectedByType;
        result.types = [];
        result.expected = [];
        for (var type in data.expectedByType){
            result.types.push(type);
            for (var i in data.expectedByType[type]){
                result.expected.push(data.expectedByType[type][i]);
            }
        }
        return result;
    }
    throw new Error("Unknown JSON response: " + data);
}
